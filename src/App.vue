...
white-space: nowrap;
  ...
  " class="ming">伪类时候的鼠标样式</span>
    ...
    <div class="new">
      ...
      <div>
        <p class="heiha">35.什么是变量、函数提升？怎么避免？</p>
        局部变量：在函数内部定义的变量，只有自己能调用
        <br>

        <button @click="jubu">局部变量</button>
        <br>
        全局变量：变量属于window对象，可以应用于所有脚本
        <br>
        <button @click="quanju">全局变量</button>
        <br>
        函数提升：创建函数的方法有，函数声明与函数字面量，只有函数声明才存在函数提升，函数提升高于变量提前
        <br>
        <button @click="hanshu">函数提升</button>
        <br>
        变量提升：变量可以在使用后声明，也就是变量可以先使用再声明。因为变量会自己提升
<br>
